...
Ve výše uvedeném příkladu jsme vytvořili \inlineDCode{mixin template MyMixin}, který jsme následně vložili do tříd \inlineDCode{C} a \inlineDCode{D}. Jak bylo uvedeno dříve, \textit{mixins} jsou velice podobné kopírování textu a~kompilátor se chová téměr stejně, jako kdybychom obsah \inlineDCode{MyMixin} přímo vložili do daných tříd. Tato vlastnost umožňuje do jisté míry suplovat vícenásobné dědění, které jazyk D~nepodporuje (místo toho nabízí \textit{interfaces}, podobně jako Java), nicméně program ztrácí na přehlednosti a bezpečnost kódu je také nižší (delší rozvedení problematiky je mimo rámec této práce).
