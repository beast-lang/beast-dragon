...
\chapter{Koncept ctime}
Tento koncept je nový pro jazyk NATI. Zasahuje do několika již známých konceptů -- například šablonové metaprogramování, reflexe jazyka, vykonávání funkcí za doby kompilace; všechny tyto koncepty spojuje do jednoho koherentního celku.

Tento koncept zavádí jednu jednoduchou myšlenku, ze které pak vyplývá zbytek. Tou myšlenkou je \textbf{zavedení klasifikátoru pro proměnné, jejichž hodnota se dá zjistit} bez nutnosti spouštět program. Tímto klasifikátorem je v~jazyce NATI dekorátor \inlineCode{@ctime}.
...
\subsection{Datové závislosti}
Uvedeme další jednoduchý příklad:
\begin{code}
@ctime Int x = console.readNumber();
\end{code}

Zde je zřejmé, že proměnná \inlineCode{x} nemůže splňovat podmínky konceptu ctime. Příkaz \inlineCode{console.readNumber()} čte data z konzole a návratová hodnota této funkce se nedá zjistit bez spuštění programu. Z tohoto příkladu vyplývá, že ctime proměnná nemůže být datově závislá na volání (alespoň některých) funkcí.
...
\end{code}
V tomto příkladě lze hodnotu proměnné \inlineCode{a} určit. Ctime proměnná tedy může být datově závislá na volání funkcí, ale jen některých.
...
Tento případ je také v pořádku

\section{Konečný přehled axiomů}