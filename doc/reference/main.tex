...
\begin{centeredRefTabular}{P{1.5cm} | c | c | p{6cm} | c}
	...
	7
		...
		\cline{2-5}
		& \inlineCode{x < y} & \inlineCode{binLt(R)} & Less than & \\
		& \inlineCode{x <= y} & \inlineCode{binLte(R)} & Less than or equal to & \multirow{3}{*}{\parbox{1cm}{\centering L$\rightarrow$R\\SPEC}} \\
		& \inlineCode{x == y} & \inlineCode{binEq(R)} & Equal to & \\
		& \inlineCode{x >= y} & \inlineCode{binGte(R)} & Greater than or equal to & \\
		& \inlineCode{x > y} & \inlineCode{binGt(R)} & Less than & \\
	\hline
