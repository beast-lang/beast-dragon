...
\begin{code}
alias, auto, break, case, class, continue, delete, else, enum, for, if, import, in, is, module, new, return, switch, this, while
\end{code}
...
\begin{enumerate}
	\item Overloads that do not match given arguments are ignored (also considering constness)
...
\begin{enumerate}
	...
	\item Find overload that has lowest-value context (as ordered in the \enumRef{DecorationContext} enum) and remove overloads that are of different context
	\item Remove those overloads whose list of required implicit casts is superset of any other overload's required implicit cast list in the overloadset (auto arguments are always considered as implicit casted)
...
\begin{code}
@ctime Type! t = Int32; // t is a mutable @ctime variable that can hold types.

t x = 5; // Now we declared a variable x of type t (which is Int32). It is a runtime variable.
