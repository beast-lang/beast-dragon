...
\begin{centeredRefTabular}{P{1.5cm} | c | c | p{6cm} | c}
	...
	1
		& \inlineCode{x(args)} & & Function call & \multirow{3}{*}{L$\rightarrow$R} \\
	...
	3
		...
		& \inlineCode{new x( args )} & & Dynamic construction & \\
	\hline
...
@public:
	auto #opUnary( @ctime Operator op, auto args ... ) {
...
\begin{code}
Boolean #opUnary( Operator.preNot ) {
	return !referencedValue;	
}
Int16 #opUnary( Operator("x++") ) {
	return referencedValue ++;
}
\end{code}
...
\begin{enumerate}
	...
	\item Resolve and call \inlineCode{x.#opUnary( op )} (or error).
\end{enumerate}
