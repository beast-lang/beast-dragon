...
\begin{grammar}	
	...
	\grDef{BitExpr} \gr{SumExpr} \\
		\grAltLn \gr{PrefixExpr} ( \kwd{>{}>} | \kwd{<{}<} | \kwd{>{}>{}>} | \kwd{<{}<{}<} ) \gr{PrefixExpr} \\
		\grAltLn \gr{PrefixExpr} \{ \kwd{\&} \gr{PrefixExpr} \}+ \\
		\grAltLn \gr{PrefixExpr} \{ \kwd{|} \gr{PrefixExpr} \}+ \\
		\grAltLn \gr{PrefixExpr} \{ \kwd{\^} \gr{PrefixExpr} \}+ \\
	...
	\grDef{VarDeclExpr} \gr{PrefixExpr} \\
		\grAltLn \gr{PrefixExpr} \token{Identifier} [ \gr{ParentCommaExpr} ] \\
		\grAltLn \kwd{new} \gr{PrefixExpr} [ \gr{ParentCommaExpr} ] \\

	\grDef{PrefixExpr} \gr{SuffixExpr} \\
		\grAltLn ( \kwd{!} | \kwd{-} | \kwd{\~} | \kwd{++} | \kwd{-{}-} ) \gr{SuffixExpr} \\
		%\grAltLn \gr{InlineVarDecl} \\
		\grAltLn \kwd{\$} \\
		
	\grDef{SuffixExpr} \gr{AtomExpr} \\
		\grAltLn \gr{AtomExpr} \{ \gr{SuffixExpr} \} \\
...
\begin{centeredRefTabular}{P{1.5cm} | c | c | p{6cm} | c}
	...
	1
		...
		& \inlineCode{x[args]} & & Array subscripting & \\
		& \inlineCode{x.ident} & & Attribute access & \\
		\cline{2-4}
		& \inlineCode{x++} & \inlineCode{suffInc} & Suffix increment & \\
		& \inlineCode{x--} & \inlineCode{suffDec} & Suffix decrement & \\
		& \inlineCode{x!} & \inlineCode{suffNot} & Mutable type & \\
	...
	2
		...
		\cline{2-5}
		& \inlineCode{\$} & & Array size (can only be used inside the \inlineCode{brackets} operator) & \\
	\hline
	...
	4
		& \inlineCode{x * y} & \inlineCode{binMult} & Multiplication & \multirow{2}{*}{L$\rightarrow$R} \\
		& \inlineCode{x / y} & \inlineCode{binDiv} & Division \\
		\cline{2-5}
		& \inlineCode{x \% y} & \inlineCode{binMod} & Modulo & NOCH \\
	\hline
	
	5
		& \inlineCode{x + y} & \inlineCode{binPlus} & Addition & \multirow{2}{*}{L$\rightarrow$R} \\
		& \inlineCode{x - y} & \inlineCode{binMinus} & Subtraction & \\
	\hline
	
	\multirow{4}{1.5cm}{\centering 6\\(cannot use P3, P4)}
		& \inlineCode{x & y} & \inlineCode{binBitAnd} & Bitwise AND & \multirow{3}{*}{\parbox{1cm}{\centering L$\rightarrow$R,\\SAME}} \\
		& \inlineCode{x | y} & \inlineCode{binBitOr} & Bitwise OR & \\
		& \inlineCode{x ^ y} & \inlineCode{binBitXor} & Bitwise XOR / power & \\
		\cline{2-5}
		& \inlineCode{x >> y} & \inlineCode{binRSft} & Bitwise right shift & \multirow{4}{*}{NOCH} \\
		& \inlineCode{x << y} & \inlineCode{binLSft} & Bitwise left shift & \\
		& \inlineCode{x >>> y} & \inlineCode{binRRot} & Bitwise right circular shift & \\
		& \inlineCode{x <<< y} & \inlineCode{binLRot} & Bitwise left circular shift & \\
	\hline
	
	7
		& \inlineCode{x is y} & \inlineCode{binIs} & Is (pattern matching) & \multirow{5}{*}{NOCH} \\
		& \inlineCode{x !is y} & \inlineCode{binIsNot} & Is not & \\
		& \inlineCode{x in y} & \inlineCode{binIn} & In & \\
		& \inlineCode{x !in y} & \inlineCode{binNotIn} & Not in & \\
		& \inlineCode{x != y} & \inlineCode{binNeq} & Not equal to & \\
		\cline{2-5}
		& \inlineCode{x < y} & \inlineCode{binLt} & Less than & \\
		& \inlineCode{x <= y} & \inlineCode{binLte} & Less than or equal to & \multirow{3}{*}{\parbox{1cm}{\centering L$\rightarrow$R\\SPEC}} \\
		& \inlineCode{x == y} & \inlineCode{binEq} & Equal to & \\
		& \inlineCode{x >= y} & \inlineCode{binGte} & Greater than or equal to & \\
		& \inlineCode{x > y} & \inlineCode{binGt} & Less than & \\
	\hline
	
	8
		& \inlineCode{x && y} & \inlineCode{binLogAnd} & Logical AND & \multirow{2}{*}{\parbox{1cm}{\centering L$\rightarrow$R,\\SAME}} \\
		& \inlineCode{x || y} & \inlineCode{binLogOr} & Logical OR & \\
	\hline
	
	9
		& \inlineCode{x .. y} & \inlineCode{binRange} & Range & NOCH \\
	\hline
	
	10
		& \inlineCode{x = y} & \inlineCode{assign} & Assignment & \multirow{2}{*}{\parbox{1cm}{\centering L$\rightarrow$R,\\SAME}} \\
		& \inlineCode{x := y} & \inlineCode{refAssign} & Reference assignment & \\
		\cline{2-5}
		& \inlineCode{x += y} & \inlineCode{binPlus} & \multirow{12}{*}{Op assign operators} & \multirow{12}{*}{NOCH} \\
		& \inlineCode{x -= y} & \inlineCode{binMinus} && \\
		& \inlineCode{x *= y} & \inlineCode{binMult} && \\
		& \inlineCode{x /= y} & \inlineCode{binDiv} && \\
		& \inlineCode{x \%= y} & \inlineCode{binMod} && \\
		& \inlineCode{x &= y} & \inlineCode{binBitAnd} && \\
		& \inlineCode{x |= y} & \inlineCode{binBitOr} && \\
		& \inlineCode{x ^= y} & \inlineCode{binBitXor} && \\
		& \inlineCode{x <<= y} & \inlineCode{binLSft} && \\
		& \inlineCode{x >>= y} & \inlineCode{binRSft} && \\
		& \inlineCode{x <<<= y} & \inlineCode{binLRot} && \\
		& \inlineCode{x >>>= y} & \inlineCode{binRRot} && \\
		\cline{2-5}
...
@public:
	enum : Set {
		prefix = preNot | preMinus | preTilde /* etc */,
		suffix = suffNot | suffMult | suffAnd /* etc */,
...
\begin{code}
auto #opXX( @ctime Operator op, auto args ... );
\end{code}
where \inlineCode{opXX} is function name (mentioned below).

So for example, you can overload all suffix operators at once using:
\begin{code}
@final class SuffixProxy( Type T ) {
...
@public:
	auto #opSuffix( @ctime Operator op, auto args ... ) {
...
\begin{code}
Boolean #opSuffix( Operator.suffNot ) {
	return !referencedValue;	
}
Int16 #opSuffix( Operator("x++") ) {
	return referencedValue ++;
}
\end{code}
...
\begin{code}
class A {
	@public @abstract Void #opAssign( Operator.binPlus, A@ other );
}

@final class B : @public A {
	@public @override Void #opAssign( Operator.binPlus, A@ other ) {
		// code here	
	}
}
\end{code}
...
\begin{enumerate}
	...
	\item If the resolution resulted an empty overloadset, try calling (left operand scope identifier resolution + overload resolution + call)\\ \inlineCode{instance.#dispatch( "ident" )}
...
@public:
	...
	auto #dispatch( @ctime String str )
...
\subsection{The '\inlineCode{x(args)}' operator}
Consider expression
\begin{code}
a.f( arg1, arg2 )
\end{code}
...
\begin{enumerate}
	\item Check if the left operand overloadset contains only one record (or error).
	\item Resolve and call \inlineCode{a.f.#call( arg1, arg2 )}.
\end{enumerate}
...
\begin{enumerate}
	\item Check if the left operand overloadset contains only one record (or error).
	\item Resolve and call \inlineCode{x.#index( arg1, arg2 )} (or error).
\end{enumerate}

\subsection{The '\inlineCode{\$}' (dollar) operator}
In the bracket parameters, you can use the \inlineCode{\$} operator.

It doesn't take any operands and translates to \inlineCode{x.#dollar( Index paramId )} where \inlineCode{paramId} is the index of the bracket parameter the dollar operator was used in (starting from 0).
...
\begin{enumerate}
	...
	\item Resolve and call \inlineCode{x.#opSuffix( op )} (or error).
\end{enumerate}
...
\begin{enumerate}
	\item Check if the operand overloadset contains only one record (or error).
	\item Resolve and call \inlineCode{x.#opPrefix( op )} (or error).
\end{enumerate}
...
\begin{enumerate}
	\item Check if the operand overloadset contains only one record (or error).
	\item Resolve and call \inlineCode{x.#opBinary( op, y )}.
	\item If previous step did not found any matching overload, resolve and call \inlineCode{y.#opBinaryR( op, x )} (or error).
\end{enumerate}

Every binary operator has a record in the \inlineCode{Operator} enum.

\subsection{Assign operator}
Consider expression
\begin{code}
	x = y
\end{code}

Here, the \inlineCode{x} is the left operand and \inlineCode{y} is the right operand.

\paragraph{Behavior}
\begin{enumerate}
	\item Check if the operand overloadset contains only one record (or error).
	\item Resolve and call \inlineCode{x.#assign( y )}.
\end{enumerate}

\subsection{Ref assign operator}
Consider expression
\begin{code}
	x := y
\end{code}
...
\begin{enumerate}
	\item Check if the operand overloadset contains only one record (or error).
	\item Resolve and call \inlineCode{x.#refAssign( y )}.
\end{enumerate}

\subsection{Operator assign operator}
Consider expression
\begin{code}
	x |= y
\end{code}

Here, the \inlineCode{x} is the left operand, \inlineCode{y} is the right operand and \inlineCode{|=} is the operator. There is a corresponding \inlineCode{Operator} enum item for each op assign operator (see \ref{opPrecedence}) in the \inlineCode{binXX} format. This enum item corresponds with the \inlineCode{op} mentioned in the behavior paragraph below.

\paragraph{Behavior}
\begin{enumerate}
	\item Check if the operand overloadset contains only one record (or error).
	\item Resolve and call \inlineCode{x.#opAssign( op, y )}.
\end{enumerate}
...
\begin{grammar}
	...
	\\
	\grDef{TypeExpr} \{ \gr{Decoration} \} ( \gr{PrefixExpr} | \gr{AutoType} ) \\
	\grDef{AutoType} \kwd{auto} [ \kwd{?} ] [ \kwd{!} ] \\
	\grDef{VarDeclCtor} ( \kwd{=} | \kwd{:=} ) \gr{Expression} \\
\end{grammar}
...
\begin{compactitem}
	...
	\item \inlineCode{Type var = Type( arg1, arg2 );}  calls \inlineCode{var.#ctor( arg1, arg2 );}. This is a special case of the previous rule and is checked during semantic analysis (the right expression must evaluate to \inlineCode{Type} constructor call).
