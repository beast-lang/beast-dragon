...
\begin{centeredRefTabular}{P{1.5cm} | c | c | p{4cm} | c}
	...
	1
		& \inlineCode{x(args)} & \inlineCode{funcCall} & Function call & Left to right \\
		& \inlineCode{x[args]} & \inlineCode{brackets} & Array subscripting & \\
		& \inlineCode{x.ident} & \inlineCode{dispatch} & Attribute access & \\
	\hline
	
	2
		& \inlineCode{!x} & \inlineCode{preNot} & Logical NOT & Cannot be chained \\
		...
		\cline{2-5}
		& \inlineCode{x++} & \inlineCode{suffInc} & Suffix increment & Cannot be chained \\
		& \inlineCode{x--} & \inlineCode{suffDec} & Suffix decrement & \\
		\cline{2-5}
		& \inlineCode{x!} & \inlineCode{suffNot} & Mutable type & Left to right \\
	...
	3
		& \inlineCode{x id( args )} & (not overloadable) & In-expression variable declaration & Cannot be chained \\
		& \inlineCode{new x( args )} & \inlineCode{new_} & Dynamic construction & \\
	\hline
	
	4
		& \inlineCode{x * y} & \inlineCode{binMult(R)} & Multiplication & Left to right \\
		& \inlineCode{x / y} & \inlineCode{binDiv(R)} & Division \\
		\cline{2-5}
		& \inlineCode{x \% y} & \inlineCode{binMod(R)} & Modulo & Cannot be chained \\
	\hline
	
	5
		& \inlineCode{x + y} & \inlineCode{binPlus(R)} & Addition & Left to right \\
		& \inlineCode{x - y} & \inlineCode{binMinus(R)} & Subtraction & \\
	\hline
	
	\multirow{4}{1.5cm}{\centering 6\\(cannot use P3, P4)}
		& \inlineCode{x & y} & \inlineCode{binAnd(R)} & Bitwise AND & Left to right, \\
		& \inlineCode{x | y} & \inlineCode{binOr(R)} & Bitwise OR & only the same \\
		& \inlineCode{x ^ y} & \inlineCode{binXor(R)} & Bitwise XOR / power & \\
		\cline{2-5}
		& \inlineCode{x >> y} & \inlineCode{binRSft(R)} & Bitwise right shift & Cannot be chained \\
	...
	7
		& \inlineCode{x is y} & \inlineCode{binIs(R)} & Is (pattern matching) & Cannot be chained \\
		...
		\cline{2-5}
		& \inlineCode{x < y} & \inlineCode{binLess(R)} & Less than & Left to right, \\
		& \inlineCode{x <= y} & \inlineCode{binLeq(R)} & Less than or equal to & specific chaining rules \\
	...
	8
		& \inlineCode{x && y} & \inlineCode{binLogAnd(R)} & Logical AND & Left to right, \\
		& \inlineCode{x || y} & \inlineCode{binLogOr(R)} & Logical OR & only the same \\
	\hline
	
	9
		& \inlineCode{x .. y} & (not overloadable) & Range & Cannot be chained \\
	\hline
	
	10
		& \inlineCode{x = y} & \inlineCode{assign(R)} & Assignment & Right to left, \\
		& \inlineCode{x := y} & \inlineCode{refAssign(R)} & Reference assignment & only the same \\
		\cline{2-5}
		& \inlineCode{x += y} & \inlineCode{plusAssign(R)} & Various assignments & Cannot be chained \\
		...
		\cline{2-5}
		& \inlineCode{x => y} & (not overloadable) & Association & Cannot be chained \\
	\hline
	
	11
		& \inlineCode{x, y} & & Comma & Left to right \\
