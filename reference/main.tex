...
\begin{grammar}
	...
	\grDef{PrntCommaExprItem} ( \gr{InexprVarDecl} | \gr{Expression} ) [ \kwd{...} ] \\
...
\begin{grammar}	
	...
	\grDef{P1Op} \gr{ParentCommaExpr} \\
		\grAltLn \kwd{[} [ \gr{RangeExpr} \{ \kwd{,} \gr{RangeExpr} \} [ \kwd{,} ] ] \kwd{]} \\
...
\begin{centeredRefTabular}{P{1.5cm} | c | c | p{5cm} | c}
	...
	2
		...
		\cline{2-5}
		& \verb|x!| & (not overloadable) & Mutable type & Left to right \\
		& \verb|x?| & (not overloadable) & Reference type & \\
		\cline{2-5}
...
@public:
	...
	{
		@ctime Offset[ Char ] translationArray = [
			'x' => 0,
			'y' => 1,
			'z' => 2
		];
...
\paragraph{Behavior}
\begin{enumerate}
	\item \textbf{Do not} check if the left operand overloadset contains only one record. This means that the \inlineCode{x.f} expression can have more overloads in the overloadset. \inlineCode{funcCall} is the \textbf{only operator that allows this,} any other operator requires its operands to have single overload.
	\item For each overload in the right operand overloadset, perform scope identifier resolution for \inlineCode{#operator}, and join all the results into one overloadset.
	\item For the overloadset from the previous step, perform an overload resolution with parameters \\\inlineCode{( Operator.funcCall, arg1, arg2 )}.
	\item Call the resulting overload (or error).
\end{enumerate}

\paragraph{Consequences}
Behavior described above makes the overloading work. \\The \inlineCode{func.#operator( Operator.funcCall, args )} function is equivalent to the function \inlineCode{func}.

There are some corner cases whose behavior might be confusing for some people:
\begin{code}
class C {
	@public Void #operator( Operator.funcCall, Int8 x ) { /* ... */	}
	@public Void #operator( Operator.funcCall, String x ) { /* ... */	}
}
class D {
	@public Void #operator( Operator.funcCall, String x ) { /* ... */	}
}
...
Void main() {
	a( 8 ); // This code is actually okay; Resolution of 'a' returns two overloads, which the operator 'x(args)' can accept and it chooses the right function overload.
	a( "asd" ); // Ambiguous; could either mean C( "asd" ) or D( "asd" )
}
\end{code}
...
\paragraph{The '\inlineCode{x ...}' operator} is used for 'expading' a value across multiple parameter slots. It does so using standard iterating process performed at ctime.
\begin{code}
Void f( Int32 a, Int32 b, Int32 c ) { /* ... */ }
Void g( String x, Int32 a ) { /* ... */ }
...
\paragraph{TODO} The \inlineCode{...} 'operator'
...
\paragraph{The dollar operator}
In the bracket parameters, you can use the \inlineCode{\$} operator. It doesn't need any operands. It translates to \inlineCode{x.#operator( Operator.dollar, Offset paramId )} where \inlineCode{paramId} is the index of the parameter the dollar operator was used in (starting from 0).
...
\paragraph{TODO} The range operator, operator parameter restrictions, overload selection, dollar operator, filtering overloads
