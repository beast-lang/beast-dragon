...
\begin{code}
alias, auto, blueprint, break, case, class, continue, debug, decorator, delegate, do, else, enum, except, finally, for, foreach, function, generic, goto, if, import, in, is, loop, module, return, scope, singleton, switch, template, this, throw, try, while, with, yield
\end{code}
...
\paragraph{Data type} Type of a literal is always the smallest signed type possible. If the number doesn't fit in the signed variant but fits in the unsigned variant with the same size, the unsigned variant is used. If the required bit count exceeds 128, a \typeRef{BigInt} type is used.
...
\paragraph{Data type} Float literal is always of type \typeRef{Float64}. It is recommended to use the \verb|f| for casting to \typeRef{Float32}.
...
\end{code}
is then called where the first argument is the \token{Identifier} used and the second is the literal value (stringified).
...
\begin{itemize}
	...
	\item A compile-time function is always evaluated in compile time. That implies that parameters must be known at compile time.
	\item A compile-time variable's value cannot be changed at runtime. A compile-time function cannot be executed at runtime.
\end{itemize}
...
A function decorated with \decoratorRef{ctime} is a compile-time function. A compile-time function is always executed at compile time. It can return a \hyperref[ctime:class]{compile-time type}. Its parameters and all variables used in its body are compile-time and the \decoratorRef{ctime} decorator should be omitted (a warning is shown otherwise).
