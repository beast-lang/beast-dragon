...
\begin{code}
alias, auto, blueprint, break, case, class, continue, debug, decorator, delegate, do, else, enum, except, false, finally, for, foreach, function, generic, goto, if, import, in, is, loop, module, return, scope, singleton, switch, template, this, throw, true, try, while, with, yield
\end{code}

\section{Literals}
\begin{grammar}
	\grDef{Literal} \gr{UnitLiteral} [ \token{Identifier} ] \\
		\grAltLn \gr{BooleanLiteral} \\
...
\begin{grammar}
	\grDef{BooleanLiteral} \kwd{true} | \kwd{false}
\end{grammar}
...
NATI supports a syntactical construct we call 'literal units'. A programmer can add an \token{Identifier} after a literal.

A function
\begin{code}
@ctime auto #literalUnit( String unit, auto literal );
\end{code}
is then called where the first argument is the \token{Identifier} used and the second is the literal value. The whole expression value is the result of the function call.
...
@ctime Length #literalUnit( String unit, Float32 value )
	if( unit in lengthUnits.keys && isNumericType( value ) )
{
	// Length is a class that stores a length in m in a Float32 type
	return Length( value * lengthUnits[ unit ] );
}

Length len = 32.5 cm;
\end{code}

\paragraph{TODO} string literals
