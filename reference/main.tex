...
\begin{grammar}
	...
	\grDef{InlineVarDecl} \{ \gr{Decoration} \} \gr{P1Expr} \token{Identifier} [ \gr{ParentCommaExpr} ] \\
	\grDef{VariableDeclStmt} \gr{P1Expr} [ \kwd{\&} ] \token{Identifier} [ \gr{VarDeclCtor} ] \\
...
\begin{grammar}
	...
	\grDef{P5Expr} \gr{P4Expr} \\
		\grAltLn \gr{P2Expr} ( \kwd{>{}>} | \kwd{<{}<} | \kwd{>{}>{}>} | \kwd{<{}<{}<} ) \gr{P2Expr} \\
...
\section{Variable declarations (in functions)}
