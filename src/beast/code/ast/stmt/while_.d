	...
	public:
		override void buildStatementCode( DeclarationEnvironment env, CodeBuilder cb ) {
			...
			cb.build_loop( ( CodeBuilder cb ) { //
				cb.build_if(  //
					condition.buildSemanticTree_singleExpect( coreLibrary.type.Bool ).expectResolveIdentifier( ID!"#opPrefix" ).resolveCall( condition, true, coreLibrary.enum_.operator.preNot ), //
					( CodeBuilder cb ) { //
						cb.build_break( );
					}, null );
