module beast.code.entity.type.type;
...
import beast.code.entity.function_.primstcnrt;
	...
	void initialize() {
		...
		mem ~= new Symbol_PrimitiveMemberNonRuntimeFunction(ID!"to", this, //
				...
				Symbol_PrimitiveMemberNonRuntimeFunction.paramsBuilder().ctArg(coreType.Type).finish( //
					...
					CallMatchSet explicitCast = inst.resolveIdentifier(ID!"#explicitCast").CallMatchSet(ast, false).arg(targetTypeEntity);
					if (auto result = explicitCast.finish())
						return result;

					CallMatchSet implicitCast = inst.resolveIdentifier(ID!"#implicitCast").CallMatchSet(ast, false).arg(targetTypeEntity);
...
public:
	final Overloadset resolveIdentifier(Identifier id, DataEntity instance, MatchLevel matchLevel = MatchLevel.fullMatch) {
		...
		if (auto result = resolveIdentifier_direct(id, instance, matchLevel))
			return result;
	...
	final Overloadset resolveIdentifier_direct(Identifier id, DataEntity instance, MatchLevel matchLevel = MatchLevel.fullMatch) {
		debug assert(initialized_, "Class '%s' not initialized".format(this.tryGetIdentificationString));
	...
	final Overloadset expectResolveIdentifier(Identifier id, DataEntity instance, MatchLevel matchLevel = MatchLevel.fullMatch) {
		if (auto result = resolveIdentifier(id, instance, matchLevel))
			return result;
