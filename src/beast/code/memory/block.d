...
final class MemoryBlock {
	...
	public:
		enum Flag {
			...
			local = 1 << 1, /// Block is local - it cannot be accessed from other sessions (should not happen at all); tested only in debug; used for local and temporary variables
			runtime = 1 << 2, /// Memory block is runtime - cannot be read/written at compile time
			functionParameter = 1 << 3, /// Memory block represents a function parameter
			contextPtr = 1 << 4, /// Memory block represents a context pointer
			interpeter = 1 << 5, /// Memory block is generated by the interpreter (> is neither ctime or runtime)
			result = 1 << 6, /// Memory block represents a result variable
		}
