...
final class MemoryBlock {
	...
	public:
		enum Flag {
			...
			interpreter = functionParameter << 1, /// Memory block is generated by the interpreter (> is neither ctime or runtime)
			result = interpreter << 1, /// Memory block represents a result variable
			dynamicallyAllocated = result << 1, /// The variable has been dynamically allocated (using malloc)
			doNotMirrorChanges = dynamicallyAllocated << 1, /// Do not mirror changes of the @ctime memory block
		}
