...
/**
	USAGE:
	- mixin this
	- implement void obtain_<<identifier>>() which does the task
	- implement function like <<Type>> <<identifier>>() { enforce_<<identifier>>(); return <<identifier>>_data; } (enforce_<<identifier>>() is generated by this mixin)
...
mixin template TaskGuard( string identifier ) {
	...
	static assert( __traits( hasMember, typeof( this ), _taskGuard_obtainFunctionName ), "You must implement 'void " ~ typeof( this ).stringof ~ "." ~ _taskGuard_obtainFunctionName ~ "()'." );
...
public:
	...
	void _taskGuard_func( ) {
		...
		try {
			__traits( getMember, this, _taskGuard_obtainFunctionName )( );
		}
	...
	mixin( "alias enforce_" ~ identifier ~ " = _taskGuard_func;" );
